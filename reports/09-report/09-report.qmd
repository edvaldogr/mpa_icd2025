...
```{r}
# Definindo a função demanda linear
demanda_linear <- function(x) {
  return(50- 2*x)
}

curve(demanda_linear, 
      from = 0, 
      to = 30, 
      xlab = "Quantidade",
      ylab = "Preço", 
      main = "Função Demanda Linear: f(x) = 50 - 2x"
      )
abline(h = 0)
abline(v = 0)
```
...
```{r}
# Definindo a função de utilidade Cobb-Douglas
utilidade_cobb <- function(x) {
  return(10*x^(0.5))
}

curve(utilidade_cobb, 
      from = 0, 
      to = 30, 
      xlab = "Quantidade",
      ylab = "Utilidade", 
...
```{r}
# Definindo a função de crescimento logístico
crescimento_logistico <- function(x) {
  return(100/(1+exp(-0.1*(x - 50))))
}

curve(crescimento_logistico, 
      from = 0, 
      to = 1000, 
      xlab = "Tempo",
      ylab = "Crescimento", 
      main = "Função de Crescimento Logístico: f(x) = 100/(1+exp(-0.1*(x - 50)))"
      )
...
curve(f, 
      ...
      xlab = "x",
      ylab = "y", 
      main = "Função: f(x) = -2*x^2 + 4*x + 1"
      )
abline(h = 0)
abline(v = 0)

# Maximização via minimização de -f(x)
resultado <- optimize(function(x) -f(x), interval = c(0, 2))
