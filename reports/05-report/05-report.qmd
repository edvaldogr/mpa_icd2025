...
```{r}
# Transforma para o formato longo usando pivot_longer
vendas_lojas_organizada <- vendas_lojas %>%
  pivot_longer(
    cols = c("T1", "T2", "T3", "T4"), # colunas que serão valores de nova coluna
    names_to = "trimestre", # nome da nova coluna
    values_to = "vendas" # nome de outra nova coluna
  )

# Visualiza o resultado (formato longo/organizado)
vendas_lojas_organizada
```
...
```{r}
# Transforma para o formato longo usando pivot_longer
investimentos_marketing_organizada <- investimentos_marketing %>%
  pivot_longer(
    cols = Jan:Mar, # colunas que serão valores de nova coluna
    names_to = "mes", # nome da nova coluna
    values_to = "investimento" # nome de outra nova coluna
  )

# Visualiza o resultado (formato longo/organizado)
investimentos_marketing_organizada
```
...
```{r}
# Transforman para o formato longo/organizado
incidadores_dptos_organizada <- incidadores_dptos %>%
  pivot_longer(
    cols = -departamento, # todas as colunas exceto "departamento"
    names_to = c("mes", "indicador"), # nomes de duas novas colunas
    names_sep = "_", # separador nos nomes das colunas
    values_to = "valor" # nome de nova coluna para receber os valores da receita
  )

# Visualiza o resultado
incidadores_dptos_organizada
```
...
```{r}
dados_paises_exercicio2 <- gapminder %>% select(-pop, -gdpPercap)
...
```{r}
dados_desafio <- gapminder %>% 
  select(contains("p")|ends_with("p")) # | = "OU Lógico" 
head(dados_desafio)
```
...
```{r}
resultado_exercicio1 <- gapminder %>%
  # Filtra por continente, ano e expectativa de vida
  filter(continent == "Americas" & year == 2007)
```
...
```{r}
pib_pais <- gapminder %>%
  mutate(pib_per_capta = gdpPercap * pop) %>%
  select(country, gdpPercap, pop,pib_per_capta)
head(pib_pais)
```
...
```{r}
pais_grande <- gapminder %>%
  mutate(
   economia_grande = ifelse(pop > 50000000, 
                       "SIM", 
                       "NÃO")
 ) %>% 
 select(country, pop, economia_grande)
head(pais_grande)
```
...
```{r}
paises_espectativa <- gapminder %>%
  filter(year == 2007) %>%
  mutate(
    espectativa_de_vida = case_when(
      lifeExp < 60 ~ "Baixa",
      lifeExp >= 60 & lifeExp <= 75 ~ "Média",
      lifeExp > 75 ~ "Alta"
    )
  ) %>%
  select(country, year, lifeExp, espectativa_de_vida)
head(paises_espectativa)
```
...
print(expectativa_por_continente, n=Inf)
```
...
```{r}
relatorio_populacao <- gapminder %>%
  filter(year == 2007) %>%  # Filtra para dados mais recentes
  group_by(continent) %>%   # Agrupa por continente
  summarize(
    Total_populacao = sum(pop),          # População total
  ) %>%
  arrange(desc(Total_populacao))  # Ordena do maior para o menor População
...
relatorio_filiais <- gapminder %>%
  group_by(continent) %>%   # Agrupa por continente
  summarize(
    Numero_de_filiais = n_distinct(country),             # Número de filiais
    Desempenho_filiais = mean(gdpPercap),         # Média do PIB per capita
    Melhor_filial = max(gdpPercap)          # PIB máximo
  )
...
```{r}
evolucao_expectativa <- gapminder %>%
  filter(continent == "Americas") %>%  # Filtra por continente América
  group_by(year) %>%   # Agrupa por continente
  summarise(expectativa_media = mean(lifeExp)) %>%
select(year, expectativa_media)


# Visualiza o resultado
evolucao_expectativa
```
...
```{r}
ranking_menor_pib <- gapminder %>%
  filter(year == 2007) %>%                # Filtra dados apenas de 2007
  select(country, continent, gdpPercap) %>% # Seleciona colunas relevantes
  arrange(gdpPercap) %>%            # Ordena do menor para o maior PIB
  head(5)                                # Mostra os 5 primeiros resultados
...
```{r}
ranking_menor_pop_america <- gapminder %>%
  filter(year == 2007 & 
           continent=="Americas") %>% # Filtra dados apenas da America
  select(country, continent, pop) %>% # Seleciona colunas relevantes
  arrange(desc(pop))            # Ordena do menor para o maior PIB
 

# Visualiza o resultado
head(ranking_menor_pop_america)
```
...
```{r}
ranking_expectativa_de_vida <- gapminder %>%
 filter(year == 2007) %>%  # Filtra para dados mais recentes
  group_by(continent) %>%   # Agrupa por continente
  summarize(
    expectativa_media = mean(lifeExp),          # Expectativa de Vida Média
  ) %>%
  select(continent, expectativa_media) %>% # Seleciona colunas relevantes
  arrange(desc(expectativa_media))  # Ordena do maior para a menor expectativa média
  
# Visualiza o resultado
ranking_expectativa_de_vida
```
...
Análise da Receita Bruta Mensal de Vendas:
