...
set.seed(123)
z <- rnorm(1000)
media_z <- sum(z>2.5)/1000
media_z
probabilidade <- 1 - pnorm(2.5)
probabilidade
...
mean(g(u))*(3-1)  # Estimativa da integral
```
```{r}
integrate(g, 1, 3)
```
...
# Integração via Monte Carlo
u <- runif(100000, min = 0, max = pi)
mean(h(u))*(pi)  # Estimativa da integral
```
